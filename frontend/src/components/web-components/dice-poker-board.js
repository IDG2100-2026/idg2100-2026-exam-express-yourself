class DicePokerBoard extends HTMLElement {
  constructor() {
    ...
    this._player1 = "Player 1";
    this._player2 = "Player 2";
    this._bestOf = "3";
    this._includeStraight = "true";
    this._diceP1 = [];
    this._diceP2 = [];
    this._round = 1;
  ...
  connectedCallback() {
    // Re-read attributes here so React has a chance to set them before we render
    ...
    // Defer so React event listeners (WS send) attach before the roll fires
  ...
  _setupDice() {
    ...
    // Dispatch board:endTurn so Game.jsx can send to WebSocket
    this.shadowRoot.getElementById("endTurn").addEventListener("click", () => {
      this.dispatchEvent(
        new CustomEvent("board:endTurn", { bubbles: true, composed: true })
      );
      this._endTurn();
    });
    ...
    this.shadowRoot.getElementById("nextRound")?.addEventListener("click", () => {
      this.dispatchEvent(
        new CustomEvent("board:matchOver", { bubbles: true, composed: true })
      );
    });
  }
  ...
  autoRoll() {
    this._tryRoll(this._activePlayer);
  }

  // Update whose turn it is and how many rolls remain
  setTurn(playerKey, rollsLeft) {
    this._activePlayer = playerKey;
    this._remainingRolls = rollsLeft ?? 3;
    this._updateTurnUI();
    this._updateButtonsUI();
  }

  // Reset board for a new round (called on round:started WS message)
  resetRound(round) {
    this._round = round != null ? round : this._round + 1;
    this._activePlayer = "player1";
    this._remainingRolls = 3;
    this._resetAllDice();
    this._updateTurnUI();
    this._updateButtonsUI();
    // Auto-roll for player1 so new round always starts with rolled dice
    setTimeout(() => this.autoRoll(), 50);
  }
  ...
  _endTurn() {
    ...
    if (this._activePlayer === "player1") {
      ...
      this._updateButtonsUI();
      this._emitTurnChanged();
      // Auto-roll for player2 so they also start with random dice
      setTimeout(() => this.autoRoll(), 50);
      return;
    }
      ...
      ${this._matchOver ? `
        <div class="overlay">
          <div class="overlay-content">
            <div class="win-subtitle">Game Over</div>
            <p class="small">${winnerName} wins the match!</p>
            <p class="small">Final score: ${this._wins.player1} - ${this._wins.player2}</p>
            <button id="nextRound">Return to Lobby</button>
          </div>
        </div>
      ` : ""}
    </div>
